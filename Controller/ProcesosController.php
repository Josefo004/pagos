...
class ProcesosController extends AppController {
     ...
     * @return void
     */
    public function pdf2($id = null, $opcion = '') {
        $this->layout = 'pdf';
        ...
        if ($proceso) {
            ...
            for ($i = 0; $i < 4; $i++) {
                ...
                $pdf->SetFont('freesans', '', 8);
                //$fecha = ($i == 0)?date('d-m-Y'):'';
                $fecha =  ($i == 0)?$this->fecha($proceso['Proceso']['fecha_emision']):'';
                $pdf->MultiCell(31, 6, 'Firma o sello', 'LB', 'C', true, 0, '', '', true, 0, false, true, 6, 'M');
                $pdf->MultiCell(31, 6, 'Fecha: ' . $fecha, 'LTB', 'L', true, 0, '', '', true, 0, false, true, 6, 'M');
                $pdf->MultiCell(31, 6, 'Firma o sello', 'LB', 'C', true, 0, '', '', true, 0, false, true, 6, 'M');
                $pdf->MultiCell(31, 6, 'Fecha:', 'LTB', 'L', true, 0, '', '', true, 0, false, true, 6, 'M');
    ...
    public function nuevo() {
        ...
        $ultimo = $this->Proceso->find('first', array('order' => array('Proceso.nro_proceso' => 'desc')));
        $up = !is_null($ultimo)? $ultimo['Proceso']['nro_proceso'] : '';
        //echo '<pre>'.var_export($ultimo,true).'</pre>';
        //echo "Ultimo Proceso ".$up;
        $this->set('ultimo',$up);
        $dependencias = $this->Proceso->Dependencia->find('list', array(
            'conditions' => array(
                'Dependencia.tipo_dependencia_id' => array(2, 4),
