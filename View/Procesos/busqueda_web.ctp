...
<div class="procesos related">
    ...
    <div class="busqueda_avanzada">
        <?php 
        echo $this->Form->create('Procesos', array('action' => 'busqueda_web', 'onsubmit' => 'return false;'));
        $size = 14;
        $maxlength = 20;
        ?>
        <table cellspacing="0" cellpadding="0">
            ...
            <tr>
                ...
                <td width="14%"><?php echo $this->Form->input('chk_ci_nit', array('type' => 'checkbox', 'value' => '1', 'label' => 'CI/NIT', $chk_ci_nit)); ?></td>
                <td width="22%"><?php echo $this->Form->input('ci_nit', array('label' => '', 'size' => $size, 'maxlength' => $maxlength, 'disabled' => !$params['chk_ci_nit'], 'default' => $params['ci_nit'])); ?></td>
                <td width="14%"><?php echo $this->Form->input('chk_beneficiario', array('type' => 'checkbox', 'value' => '1', 'label' => 'Beneficiario', $chk_beneficiario)); ?></td>
                <td width="22%"><?php echo $this->Form->input('beneficiario', array('label' => '', 'size' => $size, 'maxlength' => $maxlength, 'disabled' => !$params['chk_beneficiario'], 'default' => $params['beneficiario'])); ?></td>
            </tr>
            <?php 
    ...
    <div class="buttons left">
        ...
        <?php echo $this->Html->link($this->Html->image('botones/ver.png') . ' Ver', 'javascript:void(0);', array('onclick' => 'javascript:action(\'' . $url . '\', \'ver_web\', id_registro, null)', 'escape' => false)); ?>
    </div>
    ...
    <table cellpadding="0" cellspacing="0">
        <tr>
            ...
            <th><?php echo $this->Paginator->sort('fecha_emision', 'Fecha de Emisión'); ?></th>
...
<script type="text/javascript">
    ...
    function getURL () {
        query_ok = false;
        url = $('#ProcesosBusquedaWebForm').attr('action')
            + '/envio:si'
    ...
    $('#ProcesosBusquedaWebForm').submit(function() {
        var url = getURL();
        if (query_ok) {
            window.location = url;
        } else {
