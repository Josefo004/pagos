...
$estados = array('recibidos' => 'Recepcionados', 'enviados' => 'Enviados');
$observaciones = array('' => 'Todos', 'si' => 'Si', 'no' => 'No');
...
<div class="reportes related">
    ...
    <?php 
    echo $this->Form->create('Reportes', array('action' => 'analistas', 'onsubmit' => 'return false;'));
    ?>
    <table cellspacing="0" cellpadding="0">
        <tr>
            <td width="25%"><?php echo $this->Form->input('monto', array('label' => 'Monto', 'type' => 'select', 'options' => $montos, 'default' => $params['monto'])); ?></td>
            <td width="25%"><?php echo $this->Form->input('estado', array('label' => 'Estado', 'type' => 'select', 'options' => $estados, 'default' => $params['estado'])); ?></td>
            <!--<td width="20%"><?php // echo $this->Form->input('observaciones', array('label' => 'Con observaciones', 'type' => 'select', 'options' => $observaciones, 'default' => $params['observaciones'])); ?></td>-->
            <td width="25%"><?php echo $this->Form->input('fecha_ini', array('label' => 'Desde', 'size' => 8, 'after' => '<input id="BtnFechaIni" type="button" value="..." />', 'value' => $params['fecha_ini'], 'readonly' => true)); ?></td>
            <td width="25%"><?php echo $this->Form->input('fecha_fin', array('label' => 'Hasta', 'size' => 8, 'after' => '<input id="BtnFechaFin" type="button" value="..." />', 'value' => $params['fecha_fin'], 'readonly' => true)); ?></td>
            <td>
                <div class="right">
                <?php
...
<script type="text/javascript">
    ...
    function getURL () {
        url = '<?php echo $this->Html->url(array('controller' => 'reportes', 'action' => 'analistas')); ?>';
...
//        url += '/observaciones:' + $('#ReportesObservaciones').val();
