...
<div class="reportes related">
    ...
    <div class="buttons">
        <?php echo $this->Html->link('Por Secretarías', array('controller' => 'reportes', 'action' => 'por_secretarias'), array('class' => 'seleccionado')); ?>
    ...
    <?php 
    echo $this->Form->create('Reportes', array('action' => 'por_secretarias', 'onsubmit' => 'return false;'));
    ?>
        ...
        <tr>
            ...
            <td width="25%">
                ...
                <div id="filtro-dia" <?php echo ($params['ciclo'] == 'diario')?'':'style="display: none;"'; ?>>
                    <?php echo $this->Form->input('dia', array('label' => 'Día', 'size' => 8, 'after' => '<input id="BtnDia" type="button" value="..." />', 'value' => $params['dia'], 'readonly' => true)); ?>
                </div>
            </td>
...
<script type="text/javascript">
    ...
    function getURL () {
        url = '<?php echo $this->Html->url(array('controller' => 'reportes', 'action' => 'por_secretarias')); ?>';
    ...
    $('#ReportesPorSecretariasForm').submit(function() {
        var url = getURL();
        window.location = url;
    });
